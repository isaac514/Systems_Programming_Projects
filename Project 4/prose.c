...
int main(int argc , char** argv)
{
    ...
    //Create the pipes for transmitting stdout and stderr
    pipe(p1);
    pipe(p2);
    ...
    if (fork() == 0)
    {
        ...
        //Call to execve() with program arguments set up 
        ...
        {   
            ...
            if (selectVal > 0)
            {
                ...
                if(FD_ISSET(p1[0] , &readfd))
                {
                    ...
                    //Write contents to stdout and outfile
                    int counter1 = 0;
                    unsigned char c1;
                    char str1[5];
                    while(counter1 < bytes_read1)
                    {
                        c1 = buf1[counter1];
                        //Write to out file 
                        write(outFd , &c1 , 1);

                        //translate for stdout
                        if ((c1 < 32) || (c1 > 126))
                        {
                            sprintf(str1 , "<%02X>" , c1);
                        ...
                        //Write to stdout without translating 
                        else
                        {
                            write(1 , &c1 , 1);
                        }
                ...
                if(FD_ISSET(p2[0] , &readfd))
                {
                    ...
                    //Write contents to stderr and error file 
                    int counter2 = 0;
                    unsigned char c2;
                    char str2[5];
                    while(counter2 < bytes_read2)
                    {
                        c2 = buf2[counter2];
                        //Write to out file 
                        write(errFd , &c2 , 1);
            ...
            //gcc -Wall -pedantic -g -o prose prose.c  
        }
    }
}